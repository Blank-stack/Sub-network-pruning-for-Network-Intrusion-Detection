...
def find_random_search_thresholds(
        ...
        ceiling=0.25, max_samples=100, objective_func=None, full=True):
    """Perform a random grid search to find the best thresholds on `scores`.
    ...
    ceiling = ({'reject_total_perc': ceiling}
    ...
    while True:
        ...
        # Test with chosen thresholds
        results, h, g = test_with_rejection(
            thresholds, scores, groundtruth_labels, predicted_labels, full=full)
...
def full_search_with_constraints(
        scores, predicted_labels, groundtruth_labels, maximise_vals,
        constraint_vals, quiet=False, ncpu=-1, full=True):
