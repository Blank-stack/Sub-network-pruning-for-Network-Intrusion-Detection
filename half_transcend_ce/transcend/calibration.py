...
import half_transcend_ce.transcend.data as data
import half_transcend_ce.transcend.scores as scores


def train_calibration_set(
        X_train, y_train, n_folds=10, ncpu=1, saved_data_folder='.', loo=False, gammah=0.15, Ch=10000):
...
def train_calibration_ice(
    ...
    if os.path.exists(model_name):
        ...
        # svm = SVC(probability=True, kernel='linear''rbf', verbose=True)
        svm = SVC(kernel='rbf', gamma=0.15, C=10000)
    ...
    saved_cal_name = 'cal_fold_{}.p'.format(fold_index)
    saved_cal_name = os.path.join(saved_data_folder, saved_cal_name)
    if os.path.exists(saved_cal_name):
        pred_cal_fold = data.load_cached_data(saved_cal_name)
    else:
        pred_cal_fold = svm.predict(X_cal)
        data.cache_data(pred_cal_fold, saved_cal_name)

    # Compute p values for calibration fold

    logging.debug('Computing cal p values for fold {}...'.format(fold_index))

    saved_ncms_name_pt = 'pt_ncms_svm_cal_fold_{}.p'.format(fold_index)
    saved_ncms_name_c = 'c_ncms_svm_cal_fold_{}.p'.format(fold_index)
    saved_ncms_name_pt = os.path.join(saved_data_folder, saved_ncms_name_pt)
    saved_ncms_name_c = os.path.join(saved_data_folder, saved_ncms_name_c)

    if os.path.exists(saved_ncms_name_pt):
        ncms_proper_train_fold = data.load_cached_data(saved_ncms_name_pt)
    else:
        ncms_proper_train_fold = scores.get_svm_ncms(svm, X_proper_train,
                                                     y_proper_train)
        data.cache_data(ncms_proper_train_fold, saved_ncms_name_pt)

    if os.path.exists(saved_ncms_name_c):
        ncms_cal_fold = data.load_cached_data(saved_ncms_name_c)
    else:
        ncms_cal_fold = scores.get_svm_ncms(svm, X_cal, y_cal, fold_index)
        data.cache_data(ncms_cal_fold, saved_ncms_name_c)
...
def train_calibration_fold(params):
    ...
    if os.path.exists(model_name):
        ...
        # svm = SVC(probability=True, kernel='linear''rbf', verbose=True)
        svm = SVC(kernel='rbf', gamma=float(gammah), C=int(Ch))
    ...
    if os.path.exists(saved_ncms_name):
        ...
        ncms_proper_train_fold = scores.get_svm_ncms(svm, X_proper_train_fold,
                                                     y_proper_train_fold, str(fold_index))
        ncms_cal_fold = scores.get_svm_ncms(svm, X_cal_fold, y_cal_fold, str(fold_index))
