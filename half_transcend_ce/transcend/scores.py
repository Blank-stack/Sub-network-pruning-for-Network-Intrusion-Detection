...
def compute_p_values_cred_and_conf(
        train_ncms, groundtruth_train, test_ncms, y_test, description=''):
    ...
    cred = [compute_single_cred_p_value(train_ncms=ncm_0 if y == 0 else ncm_1, single_test_ncm=ncm)
            for ncm, y in tqdm(zip(test_ncms, y_test), total=len(y_test), desc='cred pvals ' + description)]
    conf = [compute_single_conf_p_value(train_ncms=ncm_1 if y == 0 else ncm_0, single_test_ncm=ncm)
            for ncm, y in tqdm(zip(test_ncms, y_test), total=len(y_test), desc='conf pvals ' + description)]
    # for s_cred, s_conf in zip(cred, conf):
    #     if s_cred < s_conf:
    #         print('why? :', s_cred, s_conf)

    return {'cred': cred, 'conf': conf}


def compute_single_cred_p_value(
        train_ncms, single_test_ncm):
    ...
    Args:
        train_ncms (list): An array of training NCMs to compare the
            reference point against.
        groundtruth_train (list): An array of ground truths corresponding
            to `train_ncms`.
    ...
    # faster
    how_many_are_greater_than_single_test_ncm = len([ncm for ncm in train_ncms if ncm > single_test_ncm])

    single_cred_p_value = (how_many_are_greater_than_single_test_ncm / len(train_ncms))
...
def compute_single_conf_p_value(
    ...
    single_cred_p_value_opposite_class = compute_single_cred_p_value(
        train_ncms=train_ncms, single_test_ncm=single_test_ncm_opposite_class)
